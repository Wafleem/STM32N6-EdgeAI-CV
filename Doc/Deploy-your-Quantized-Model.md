...
3. Convert the binary weights to an Intel HEX file so they can be programmed into the board’s external flash.

After generation, your new model is ready to run on-device. Next, update the application configuration.

## Update Application Configuration

Image pre-processing is handled by DCMIPP and automatically adapts to the model using defines created by `stedgeai`. Post-processing, however, must be configured by the user. Edit the Nucleo board config in [Application/NUCLEO-N657X0-Q/Inc/app_config.h](../Application/NUCLEO-N657X0-Q/Inc/app_config.h).
